...
CLIENT_SECRETS_FILE = "/usr/local/bigbluebutton/core/scripts/post_publish/client_secrets.json"
# File where we will store authentication credentials after acquiring them.
CREDENTIALS_FILE = "/usr/local/bigbluebutton/core/scripts/post_publish/upload.py-oauth2.json"
...
def args_variable(FILE_NAME, TITLE_NAME):
  info = {'category':'22', 'description':'', 'file':FILE_NAME, 'privacyStatus':'unlisted', 'title':TITLE_NAME}
  return info
...
def get_status_upload(meetingid):
  ...
  base_youtube_path = status_path+youtube_status

  if os.path.exists(base_youtube_path+meetingid+"-youtube.done"):
    status = False
  return status

def create_status_upload_youtube(meetingid):
  status_path = "/var/bigbluebutton/recording/status/"
  youtube_status = "youtube/"
  base_youtube_path = status_path+youtube_status
  if not os.path.exists(base_youtube_path):
    os.mkdir(base_youtube_path)

  os.mknod(base_youtube_path+meetingid+"-youtube.done")
...
if __name__ == '__main__':
  ...
  if not os.path.exists(oauth2_path):
    ...
    if get_status_upload(args.meetingid):
      youtube = get_authenticated_service(args)
      try:
        file_url_videos = find("/var/bigbluebutton/published/presentation/"+args.meetingid)
